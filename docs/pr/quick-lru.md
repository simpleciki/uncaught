...
`fp-08-class.test.js` (11 tests) covers correctness of every public read path under that state. Setup: `maxSize: 2`, `Date.now` frozen, fill both slots to trigger rotation, then re-insert key `a` with a longer `maxAge`. Key `a` gets a live copy in `#cache` (value `99`, 9 000 ms) alongside the stale copy in `#oldCache` (value `1`, 100 ms). The tests check that `get`, `peek`, `expiresIn`, `values`, `entries`, `entriesAscending`, `entriesDescending`, `forEach` and `Symbol.iterator` return the live value, that `has` reports the key as present, and that `keys` includes it.
